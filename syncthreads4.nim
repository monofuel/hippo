...
var
  ...
  shared {.global.}: array[NUM_WORKERS, int] # "Shared memory" for partial sums
  finalResult {.global.}: int = 0  # Final dot product result
...
# ──────────────────────────────────────────────────────────────────────────────
# Worker "kernel" – now a dot product with parallel compute + reduction by tid=0
# ──────────────────────────────────────────────────────────────────────────────
...
proc worker(id: int): Task =
  return iterator (): bool =
    ...
    # ── BARRIER (sync before reduction) ───────────────────────────────────────
    syncthreads()

    # ── Reduction by thread 0 ─────────────────────────────────────────────────
    if threadIdxX == 0:
      var dot = 0
      for i in 0..<blockDimX:
        dot += shared[i]
        echo &"[w{threadIdxX}] adding shared[{i}] = {shared[i]}"
      finalResult = dot
      echo &"[w{threadIdxX}] final dot product = {dot}"

    # ── Optional: another barrier if more work after reduction ────────────────
    # syncthreads()
